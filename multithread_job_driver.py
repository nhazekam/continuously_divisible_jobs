...
def run_jobs(src_job, size, count, time_limit, batch=True, dynamic=False):
    start = time.time()
    ...
    for i in range(0, cores):
        threads.append(Thread(target=run_job, args=(src_q, dest_q,)))
    ...
    sys_time = end - start
    avail_time = sys_time * len(threads) # Available compute time is relative to the total resources used
...
if __name__ == "__main__":
    ...
    (output_job, sys_time, avail_time, split_time, join_time) = run_jobs(job, args.size, args.count, args.time)
